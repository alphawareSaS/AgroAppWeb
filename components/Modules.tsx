...
const Modules: React.FC = () => {
  ...
  return (
    <section id="modulos" className="py-16 sm:py-24 lg:py-32 bg-gradient-to-b from-emerald-50 via-white to-emerald-50/40 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div className="text-center mb-12 sm:mb-20 lg:mb-24">
          ...
          <h3 className="text-3xl sm:text-4xl lg:text-5xl font-black text-gray-900">{t('modules.title_start')} <span className="text-emerald-600">{t('modules.title_highlight')}</span></h3>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
          {modules.map((m, idx) => (
            <div key={idx} className="group bg-white p-6 sm:p-8 rounded-3xl sm:rounded-[2.5rem] border border-emerald-100 hover:bg-emerald-600 hover:text-white hover:border-emerald-600 transition-all duration-500 hover:shadow-2xl hover:-translate-y-2">
              <div className="w-14 h-14 sm:w-16 sm:h-16 bg-white rounded-2xl flex items-center justify-center text-emerald-600 mb-6 sm:mb-8 shadow-sm group-hover:scale-110 transition-transform">
                {m.icon}
              </div>
              ...
              <h4 className="text-xl sm:text-2xl font-bold mb-4">{m.title}</h4>
              <ul className="space-y-2 text-gray-500 group-hover:text-emerald-50 leading-relaxed font-medium text-sm sm:text-base">
